...
class HuobiAPIOrderBookDataSourceUnitTests(unittest.TestCase):
    ...
    def test_listen_for_subscriptions_successful_subbed(self, ws_connect_mock):
        ...
        self.mocking_assistant.add_websocket_aiohttp_message(
            ws_connect_mock.return_value, message=self._compress(subbed_message_snapshot), message_type=aiohttp.WSMsgType.BINARY
        )
        self.mocking_assistant.add_websocket_aiohttp_message(
            ws_connect_mock.return_value, message=self._compress(subbed_message_trade), message_type=aiohttp.WSMsgType.BINARY
        )
    ...
    def test_listen_for_order_book_diffs_logs_exception(self):
        ...
        mock_queue.put_nowait(orderbook_message)
        mock_queue.put_nowait(asyncio.CancelledError())
